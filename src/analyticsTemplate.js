...
//
// Behaviorally-flagged bots never reach this template at all (see chalk's
// analytics-core.js BOT_DETECTORS / index.js) - they're excluded from
// recentHits entirely, not just unmarked, so every hit rendered here already
// cleared both the static and behavioral bot filters.
...
const renderLogs = () => {
  ...
  if (activeIp) {
    ...
    const html = sessions.flatMap(session =>
      ...
      return \`<div class="session-header" onclick="clearFilter()" style="cursor:pointer">\` +
        ...
        \`<span class="log-path" title="\${hit.status ? \`\${hit.status} · \` : ''}\${escapeHtml(hit.path)}">\${escapeHtml(hit.path)}</span>\` +
  ...
  const html = allSessions.slice(0, 999).map(session => {
    ...
    return \`<div class="session-header">\` +
      ...
      \`<span class="log-path" title="\${firstHit.status ? \`\${firstHit.status} · \` : ''}\${escapeHtml(firstHit.path)}">\${escapeHtml(firstHit.path)}</span>\` +
